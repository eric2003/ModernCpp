...
constexpr symbolic_expression < sin_symbol , Arg > sin ( Arg ) noexcept { return {};}
...
int main(int argc, char* argv[])
{
	...
	// Writing a formula
	formula f = a;
	symbolic_expression< std::plus<void>, decltype(a), decltype(w) > hhh= a+w;
	//auto g = a + w;
	//formula g = a + w;
	//formula f = a + w;
	//formula f = a * sin(w * t + phi );
	
	// Computing the result : the following prints −3.78401
	//std::cout << f(a = 5.0 , w = 2.5 , t = 1.6 , phi = 0) << std::endl;
